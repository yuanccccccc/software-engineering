...
@data_process_bp.route("/api/sankey")
def sankey_data():
    ...
    links = [
        {"source": s, "target": t, "value": v} for (s, t), v in link_counts.items()
    ]
    nodes = list({n for s, t in link_counts for n in (s, t)})

    return jsonify({"nodes": [{"name": name} for name in nodes], "links": links})


@data_process_bp.route("/api/province_trend")
...
@data_process_bp.route("/api/province_total")
def province_total():
    df = pd.read_csv("all_water_quality.csv")
    ...
    result = [
        {"name": row["省份"], "value": int(row["count"])}
...
@data_process_bp.route("/api/pie_data")
def pie_data():
    df = pd.read_csv("all_water_quality.csv")
    df = df[df["水质类别"] != "*"]
    df = df[["省份", "流域", "水质类别"]].dropna()
    ...
    for i, basin in enumerate(top3_basins):
        ...
        pie_data = {"title": f"{basin}", "color": colors[i % len(colors)], "data": []}
...
# 原来的 /api/water 接口也保留
@data_process_bp.route("/api/water")
def water_data():
    df = pd.read_csv("all_water_quality.csv")
...
@data_process_bp.route("/api/eight_radar")
def get_top8_radar():
    print("right route")
    ...
    for i, basin in enumerate(top8_basins):
        ...
        avg_vals = [
            round(val, 2) for val in basin_df[needed_columns[1:]].mean().tolist()
        ]
...
@data_process_bp.route("/api/filter_options", methods=["GET"])
...
@data_process_bp.route("/api/water_quality", methods=["GET"])
...
@data_process_bp.route("/api/section_status")
def section_status():
    df = pd.read_csv("all_water_quality.csv")
...
@data_process_bp.route("/api/section_options")
def section_options():
    df = pd.read_csv("all_water_quality.csv")
    provinces = sorted(df["省份"].dropna().unique().tolist())
